...
| `dapr_placement.disseminateTimeout` | Sets the period of time in which a dissemination is considered failed if not completed. Any daprds which have not responded within this time will be considered non-responsive and kicked. Higher values will increase the grace period to complete dissemination in case of network issues, but reduce the chance of skipping daprds which are slow to respond. | `8s` |
...
| `dapr_scheduler.workers`                  | Number of workers (Go routines) active to push job triggers to dapr runtimes. | `2048`                                    |
